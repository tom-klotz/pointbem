...
int main(int argc,char **argv)
{
  ...
  */
  ...
  //solve SVD
  ierr = PetscPrintf(PETSC_COMM_WORLD, "Starting SVD solve...\n");CHKERRQ(ierr);
  ierr = SVDSolve(svd);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD, "SVD solve complete.\n");CHKERRQ(ierr);

  PetscInt nconvsvd;
  PetscReal sigma;
  ierr = SVDGetConverged(svd, &nconvsvd);CHKERRQ(ierr);
  if(nconv > 0) {
    ierr = SVDGetSingularTriplet(svd, 0, &sigma, NULL, NULL);CHKERRQ(ierr);
    ierr = PetscPrintf(PETSC_COMM_WORLD, "The largest singular value is %5.5f\n", sigma);CHKERRQ(ierr);
  }
  else {
